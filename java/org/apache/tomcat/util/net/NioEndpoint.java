...
public class NioEndpoint extends AbstractJsseEndpoint<NioChannel> {
    ...
    public void startInternal() throws Exception {
        ...
        if (!running) {
            ...
            pollers = new Poller[getPollerThreadCount()]; //用于检查selector中可以被处理的channel
            for (int i=0; i<pollers.length; i++) {
        ...
        public void run() {
            ...
            while (true) {
                ...
                try {
                    if (!close) {
                        hasEvents = events();
                        //获取selector中是否有事件需要处理，如果返回值>0，则表示存在一个或多个socket需要处理
                        //说名我们有请求需要处理了
                ...
                //keyCount>0代表有socket就绪需要被处理，selectedKeys中保存的Nio中就绪通道的集合
            ...
            try {
                ...
                if (handshake == 0) {
                    ...
                    if (event == null) {
                        ...
                        //连接器中的Processor过程，从socket请求中获取请求和响应
